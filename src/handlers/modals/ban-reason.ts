...
export async function handleBanReasonModal(
  ...
  // Extract ban reason from modal components
  ...
  // Process the ban in the background
  ctx.waitUntil(
    processBan(interaction, env, targetUserId, targetUsername, moderatorId, moderatorName, reason, logger)
  );

  // Immediately update the confirmation message to show processing
  return Response.json({
...
/**
 * Process the ban operation in the background
 */
async function processBan(
  ...
  try {
    ...
    if (!result.success) {
      // Send follow-up message about failure
    ...
    // Send success message to moderation channel
    if (env.MODERATION_CHANNEL_ID) {
      await sendMessage(env.DISCORD_TOKEN, env.MODERATION_CHANNEL_ID, {
        embeds: [
          {
    ...
    // Send error message to moderation channel
    if (env.MODERATION_CHANNEL_ID) {
      await sendMessage(env.DISCORD_TOKEN, env.MODERATION_CHANNEL_ID, {
        embeds: [
          errorEmbed(
